...
export function TimerWindow() {
  ...
  const ring = useMemo(() => {
    ...
    return {
      ...
      isWarning: timer.remainingMs <= 60000 && timer.remainingMs > 0 && timer.isRunning,
    }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '20px 16px 16px', gap: 16 }}>
      ...
      <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <svg viewBox="0 0 200 200" width={200} height={200}
          style={{ transform: 'rotate(-90deg)' }} role="presentation">
          <circle cx={100} cy={100} r={RADIUS} fill="none"
            stroke="var(--timer-track)" strokeWidth={14} />
          <circle cx={100} cy={100} r={RADIUS} fill="none"
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', justifyContent: 'center' }}>
        {PRESETS.map(min => (
          <button key={min} type="button"
            ...
            style={presetBtn(currentPresetMinutes === min)}>
            {min} min
          </button>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
        {!timer.isRunning ? (
          ...
          <button type="button" onClick={() => actions.pauseTimer()} style={primaryBtn}>
            Pausa
          </button>
        )}
        <button type="button" onClick={() => actions.resetTimer()} style={secondaryBtn}>
          Nollställ
        </button>
      </div>
      ...
      <div style={{
        ...
        />
        <button type="button" onClick={() => applyPreset(customMinutes)} style={secondaryBtn}>
          Sätt
        </button>
      </div>
    </div>
